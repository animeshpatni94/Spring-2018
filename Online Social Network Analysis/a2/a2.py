...
def combination(w,feats): 
    ...
    for i in combinations(w,2):
        ...
        for j in parts:
            if str1+j not in feats:
                feats["token_pair="+j] = 1
            elif str1+j in feats:
...
def vectorize_short_answer(tokens_list, feature_fns, min_freq, vocab=None):
    #from sklearn import preprocessing for running this function
    ###TODO
    ...
    for i in set(accuracy):
        ...
        if 'features' in i[0]:
            ...
            for k in i[1]:
                z.append(k.__name__)
            fname = " ".join(z)
            f.append((np.mean(acc), fname))
        else:
            w=str(i[1])
            f.append((np.mean(acc), (i[0] + "=" + w)))
    ...
    for i in range(0,len(p)):
        ...
        if(p[i] != test_labels[i]):
            if p[i] == 0:
                ...
                d['label'] = test_labels[i]
            new_list.append(d)
    new_list = sorted(new_list,key=lambda x:-x['true'])
    for i in new_list[:n]:
